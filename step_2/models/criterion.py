...
def knowledge_distillation_loss(s_logits, t_logits, T = 9):
        distillation_loss = nn.KLDivLoss(reduction='batchmean')(F.log_softmax(s_logits/T, dim=1), F.softmax(t_logits/T, dim=1)) * (T * T)
...
def get_bbox_loss(fakeIm, realIm, bboxes, batch, image_size):
    abs_rel = a1 = a2 = a3 = rmse = mse = psnr__ =0
    for i in range(batch):
        bb = bboxes.detach()[i]
        bb = [int(bb[0]*image_size),int(bb[1]*image_size),int(bb[2]*image_size),int(bb[3]*image_size)]
        pred = fakeIm[i][:,bb[1]:bb[3],bb[0]:bb[2]]
        gt = realIm[i][:,bb[1]:bb[3],bb[0]:bb[2]]
        ...
        a1_ = (thresh < 1.25     ).mean()
        a2_ = (thresh < 1.25 ** 2).mean()
        a3_ = (thresh < 1.25 ** 3).mean()
        abs_rel_ = np.mean(np.abs(gt - pred) / gt)
        psnr_ = psnr(gt,pred)
    ...
    return abs_rel, a1, a2, a3, rmse, mse , psnr__


def get_bbox_loss_1(fakeIm, realIm, bboxes, batch, image_size):
    abs_rel = a1 = a2 = a3 = rmse = mse = psnr__ =0
    for i in range(batch):
        bb = bboxes.detach()[i]
        bb = [int(bb[0]*image_size),int(bb[1]*image_size),int(bb[2]*image_size),int(bb[3]*image_size)]
        array_1 = np.ones((bb[3]-bb[1],bb[2]-bb[0]))
        pred = fakeIm[i][:,bb[1]:bb[3],bb[0]:bb[2]]
        pred = array_1 - pred 
        gt = realIm[i][:,bb[1]:bb[3],bb[0]:bb[2]]
        gt = array_1 - gt
